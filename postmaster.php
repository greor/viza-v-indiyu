<?php
$EMAIL = 'artnabatov@yandex.ru'; //  Ваш e-mail на который будут приходить заявки
$FROM = 'mail@hitopt24.ru';  //  e-mail отправителя (иногда требуется указать разрешенный настройками e-mail)
$REPLY = 'artnabatov@yandex.ru';   //  e-mail для ответа 
...
switch (@$_POST['action']) { // рассматриваем варианты разных значений поля action
    ...
    break;
    case 'callme_order':
        sendmail("Заказ обратного звонка с HitOpt24.ru", render(
                        '<div><b>Имя:</b> '.stripinput($_POST['name']).'</div>'.
